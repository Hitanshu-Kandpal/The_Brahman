...
export function CinematicEntry({ onScrollProgress }: CinematicEntryProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scale = 1 - scrollProgress * 0.62; // Shrinks to 0.38
      const y = scrollProgress * 100;

      // Apply scroll transform - keep it simple and centered
      if (titleRef.current) {
        titleRef.current.style.transform = `translate(-50%, calc(-50% + ${y}px)) scale(${scale})`;
      }
